...
def convertHeader(element):
    return process(element, "<title%s>$(header/title)</title>" % (wattrs(element)))

# FIXME: Header/heading confusion
...
def convertSection(element):
    level = int(element.getAttributeNS(None, "depth")) + 1
    return process(element,
                   '<div class="section"><a class="link">'
                   + '<h%d class="heading">$(header)</h%d></a>' % (level, level)
...
def convertBlock(element):
    ...
    css_class = ""
    if title:
        css_class = " class='ann%s'" % (
            element.getAttributeNS(None, "type").capitalize())
